...
namespace Frank.UblSharp.Internals.XsdCodeGenerator
{
    ...
    public class AdditionalFilesHelperGenerator : ISourceGenerator
    {
        ...
        private static void GenerateResourceHelperClass(GeneratorExecutionContext context)
        {
            ...
            if (!TryGetProjectDir(context, out var projectDir) || !TryGetRootNamespace(context, out var rootNamespace))
            {
                return;
            }
                                ...
                                SyntaxFactory.SingletonSeparatedList(
                                    SyntaxFactory.Argument(
                                        SyntaxFactory.LiteralExpression(SyntaxKind.StringLiteralExpression,
                                            SyntaxFactory.Literal(Path.Combine(projectDir, member.RelativePath)))))))))));
        ...
        private static ClassDeclarationSyntax GenerateClassRecursively(ClassMember classMember)
        {
            ...
            foreach (var nestedClass in classMember.NestedClasses.Values)
            {
                classDeclaration = classDeclaration.AddMembers(GenerateClassRecursively(nestedClass));
            }
